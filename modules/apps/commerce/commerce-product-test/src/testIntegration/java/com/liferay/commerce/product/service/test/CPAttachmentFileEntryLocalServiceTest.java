...
public class CPAttachmentFileEntryLocalServiceTest {
	...
	@Test
	public void testCanOnlyUpdateCorrectProductAttachment() throws Exception {
		frutillaRule.scenario(
			...
			"the first product's attachment is not updated and the second " +
				"attachment is posted correctly"
		);
		...
		CPAttachmentFileEntry postCPAttachmentFileEntry =
		...
		CPAttachmentFileEntry getCPAttachmentFileEntry1 =
			_cpAttachmentFileEntryLocalService.getCPAttachmentFileEntry(
				cpAttachmentFileEntry.getCPAttachmentFileEntryId());

		Assert.assertEquals(
			cpAttachmentFileEntry.getTitleMap(),
			getCPAttachmentFileEntry1.getTitleMap());
		Assert.assertEquals(
			cpAttachmentFileEntry.getPriority(),
			getCPAttachmentFileEntry1.getPriority(), 0.001);

		List<CPAttachmentFileEntry> cpAttachmentFileEntries =
			cpDefinition2.getCPAttachmentFileEntries(
				CPAttachmentFileEntryConstants.TYPE_OTHER,
				WorkflowConstants.STATUS_ANY);

		CPAttachmentFileEntry getCPAttachmentFileEntry2 =
			cpAttachmentFileEntries.get(0);

		Assert.assertEquals(
			postCPAttachmentFileEntry.getTitleMap(),
			getCPAttachmentFileEntry2.getTitleMap());
		Assert.assertEquals(
			postCPAttachmentFileEntry.getPriority(),
			getCPAttachmentFileEntry2.getPriority(), 0.001);
	}
