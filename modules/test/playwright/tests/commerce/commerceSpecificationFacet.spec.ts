...
test('can sort specifications by specification group priority', async ({
	...
	const optionCategory1 =
		await _apiHelpers.headlessCommerceAdminCatalog.postOptionCategory(
	...
	const specification1 =
		await _apiHelpers.headlessCommerceAdminCatalog.postSpecification(
			getSpecification(
				optionCategory1.id,
				optionCategory1.key,
	...
	await _layoutsPage.goToPages();

	await expect(_specificationFacetsPage.pageLabel).not.toBeVisible({
		timeout: 60 * 1000,
	});
});
